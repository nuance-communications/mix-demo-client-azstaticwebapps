...
export class DlgClient extends EventEmitter {
  ...
  connect(){
    ...
    return new Promise((resolve, reject) => {
      if(client._dlgaas && client._dlgaas_stream){
        resolve()
        return
      }
      ...
      try{
        client._call = client._dlgaas.start(req, 
          metadata, (error, response) => {
            ...
            console.log('[start] resolving with', ret, resolve)
            resolve(ret)
          }
        )
      } catch (ex) {
      ...
      try{
        client._call = client._dlgaas.execute(req, 
          metadata, (error, response) => {
            ...
            console.log('[execute] response -> ', ret, resolve)
            resolve(ret)
          }
        )
      } catch (ex) {
  ...
  async executeStream(streamInput, clientID){
    console.log('[dlg] executeStream')
    const client = this
    const token = await client._tokenResolver()
    return new Promise((resolve, reject) => {
      console.log('[dlg] stream got token, establishing call')
      let metadata = {
        'Authorization': `Bearer ${token.access_token}`,
      ...
      client._call.on('data', client.messageHandler.bind(client))
      client._call.on('end', status => {
        console.info('client call stream ended')
        client._recoStarted = false
        client.clearBuffer()
        client.emit('end', status)
      })
      client._call.on('status', status => {
        console.info('client call stream status update', status)
        client.emit('status', status)
      })
      client._call.on('error', error => {
        console.error('gRPC executeStream call errored', error)
        client._recoStarted = false
        client.emit('error', error)
        reject()
      })
      ...
      if(client._requestBuffer.length){
        ...
        for (const audioChunkReq of client._requestBuffer){
          console.log("[dlg] sending chunk..")
          client._call.write(audioChunkReq)
        }
        client.clearBuffer()
      }
      resolve('[execute] Started Stream Input Request', client._call)
    })
  }
  ...
  async cancel(){
    console.log('canceling current call')
    if(this._call){
      this._call.cancel()
      return true
    }
...
export class DlgController extends EventEmitter {
  ...
  wireUpClientEvents(){
    ...
    this._client.on('status', status => {
      ...
      if(status.code === 2){
        console.log('bad response -- detaching audio (source)')
        controller.clearAudioHandler()
  ...
  async executeStream({sessionId, asrSettings, clientID}){
    console.log('[dlg] starting execution stream')
    if(!this._client){
      return null
    }
    ...
    if(this._audioSource){
      console.log('[dlg] attaching audio input source for stream')
  ...
  async stopExecuteStream(){
    console.log('[dlg] stopping execution stream')
    if(this._audioSource){
      console.info('[dlg] detach audio input source for stream')
      this._audioSource.off('audio', this._onAudioHandler)
    }
    if(this._client){
      console.info('[dlg] call client end')
      this._client.end()
    }
  ...
  async stop(req){
    console.log('[dlg] calling dlgaas stop')
    if(!this._client){
      console.warn('no client to cal')
      return null
    }
    if(!req){
      return null
    ...
    if(!req.sessionId){
      console.warn('no sessionId to stop')
      return null
    }
    let stopReq = new StopRequest()
    stopReq.setSessionId(req.sessionId)
    let ret = null
    console.info('[dlgaas] stop', stopReq)
  ...
  dispose(){
    this.stop()
    if(this._audioSource){
      console.info('[dlg] disposing -> detach audio')
      this._audioSource.off('audio', this._onAudioHandler)
    }
